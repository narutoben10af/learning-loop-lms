...
Current status: the workspace/course contract, My workspace dashboard, selected-course navigation shell, teaching-focused Home, Modules integration, profile-linked synthetic People roster, role-safe Announcements, and local Files/media workflow are implemented. The immediate usable course shell is therefore concrete. The organisation question-bank and course-assessment contracts are the active domain-first delivery, followed immediately by the real teacher authoring/assembly/release and student attempt/result surface. Detailed marking and Gradebook remain later focused slices. Other named course operations stay visible with honest scope and a useful adjacent action rather than pretend functionality.
